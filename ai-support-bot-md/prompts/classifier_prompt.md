...
## 【最優先ルール】GA4・他ツールとの比較がある場合

ユーザーの発話に以下のパターンが**いずれか1つでも**含まれる場合、質問の形式にかかわらず必ず **report_difference** を選択すること。

- 「GA4では確認できない」「GA4に該当データがない」「GA4と一致しない」「GA4と差異がある」「GA4と異なる」
- 「どのように集計されているか」+「GA4」が同時に存在する
- Ptengine のデータと GA4（または他の分析ツール）のデータを比較している文脈

このルールは「どのように集計されているか教えてください」という形式が usage_guidance に見えても適用する。
