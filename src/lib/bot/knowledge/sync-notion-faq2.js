...
const SOURCE_NAME = "notion_faq2";
const SOURCE_TYPE = "notion_faq2";
...
function getDefaultPublishedToBot() {
  return process.env.NOTION_FAQ2_DEFAULT_PUBLISHED_TO_BOT !== "false";
}

export function buildChunkId(pageId) {
  return `notion_faq2_${pageId}`;
}
...
async function pageToChunk(page) {
  ...
  if (!body) {
    try {
      ...
      console.warn(`[notion faq2] body blocks 取得失敗 page_id=${page.id}: ${err.message}`);
      bodySource = "empty";
    }
  }
...
async function ensureSourceRegistered(sourcesTableId) {
  ...
  try {
    const data = await listRecords(sourcesTableId, {
      where: "(source_type,eq,notion_faq2)",
      limit: 1
    });
  ...
  if (existing) {
    ...
    await createRecord(sourcesTableId, {
      source_name: SOURCE_NAME,
      source_type: SOURCE_TYPE,
      source_url_or_path: `notion:${process.env.NOTION_FAQ2_DATABASE_ID ?? ""}`,
      is_active: true,
      freshness_status: "fresh",
      last_synced_at: now
    });
...
/**
 * Notion FAQ2 (FAQベース) DB → NocoDB knowledge_chunks の同期を実行する。
 * @returns {{ fetched: number, created: number, updated: number, skipped: number, failed: number }}
 */
export async function syncNotionFaq2() {
  ...
  console.log(`[sync notion_faq2] 開始 database=${databaseId} chunks_table=${chunksTableId}`);

  let pages;
  try {
    pages = await queryDatabase(databaseId);
  } catch (err) {
    throw new Error(`Notion FAQ2 ページ取得失敗: ${err.message}`);
  }

  console.log(`[sync notion_faq2] ${pages.length} ページ取得`);
  ...
  for (const page of pages) {
    ...
    try {
      ...
      console.warn(`[sync notion_faq2] chunk 生成失敗 page_id=${pageId}: ${err.message}`);
      stats.failed++;
      continue;
    }
    ...
    if (bodyLength === 0) {
      console.log(`[sync notion_faq2] skip (empty_body) page_id=${pageId} title="${chunk.title}" body_source=${bodySource}`);
      stats.skipped++;
      continue;
    }
    ...
    try {
      ...
      console.warn(`[sync notion_faq2] upsert 失敗 ${chunk.chunk_id}: ${err.message}`);
      stats.failed++;
    }
  }
  ...
  if (sourcesTableId) {
    try {
      ...
      console.warn(`[sync notion_faq2] knowledge_sources 更新失敗 (非致命的): ${err.message}`);
    }
  }

  console.log(
    `[sync notion_faq2] 完了 fetched=${stats.fetched} created=${stats.created} updated=${stats.updated} skipped=${stats.skipped} failed=${stats.failed}`
  );
